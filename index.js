...
async function procesarBuffer(senderId) {
  ...
  try {
    ...
    // existe esa reserva ANTES de aplicar la transición — si no se
    // encuentra nada, se cancela la transición (transicion = null) para
    // que el lead NO entre a la etapa destino (ej. "Agendo") todavía. La
    // nota SÍ se agrega igual al prompt de la etapa en la que se quedó,
    // para que le pida con calidez el día/hora exacto — y como la
    // condición de esta misma transición se vuelve a evaluar en cada
    // mensaje siguiente (con el historial completo como contexto), en
    // cuanto el lead conteste con la fecha se vuelve a intentar la
    // verificación, esta vez por fecha/hora, hasta que sea real.
    ...
    if (transicion?.verificar_calendly) {
      ...
      if (resultado.encontrado) {
        ...
        console.log(`❌ No se encontró ninguna reserva en Calendly para ${senderId} (@${conv.username || "sin username"}): ${resultado.motivo} — se cancela la transición, no entra a la etapa destino todavía.`);
    ...
    // usa Calendly en absoluto. También bloquea la transición si no
    // encuentra nada.
    ...
    if (transicion?.verificar_reserva_propia) {
      ...
      if (resultado.encontrado) {
        ...
        console.log(`❌ No se encontró ninguna reserva propia para ${senderId} (@${conv.username || "sin username"}): ${resultado.motivo} — se cancela la transición, no entra a la etapa destino todavía.`);
    ...
    if (pedirConfirmacionCalendly) {
      promptSistema += `\n\nNOTA IMPORTANTE PARA ESTE MENSAJE: el cliente dice que ya agendó su llamada, pero nuestro sistema NO encuentra ninguna reserva confirmada a su nombre en Calendly en este momento — ni siquiera buscando por el día/hora que haya mencionado hasta ahora (incluido este mismo mensaje, si trae una fecha). NUNCA des por confirmada la cita ni felicites por ella, sin importar qué tan seguro o específico suene el cliente sobre el día y la hora — la única fuente de verdad es esta nota, no lo que él diga. NO le digas que "no encuentras nada" ni que "el sistema no lo detecta" (suena robótico y desconfiado). Si todavía NO te ha dicho exactamente qué día y hora eligió, pregúntaselo con calidez, como si quisieras anotarlo tú mismo para tenerlo presente. Si YA te dijo el día y la hora (en este mensaje o en uno anterior) y aun así no aparece confirmada, no le vuelvas a preguntar lo mismo — en vez de eso, explícale con calidez y sin sonar acusador que su agenda no quedó completa: es un error común, a veces se entra al calendario y se elige el día, pero falta confirmar el paso final, y por eso no se guardó. Reenvíale el enlace del calendario y anímalo con calidez a completarlo bien esta vez.`;
    }
    ...
    if (pedirConfirmacionReservaPropia) {
      promptSistema += `\n\nNOTA IMPORTANTE PARA ESTE MENSAJE: el cliente dice que ya agendó su llamada, pero nuestro sistema NO encuentra ninguna reserva confirmada a su nombre en el calendario en este momento — ni siquiera buscando por el día/hora que haya mencionado hasta ahora (incluido este mismo mensaje, si trae una fecha). NUNCA des por confirmada la cita ni felicites por ella, sin importar qué tan seguro o específico suene el cliente sobre el día y la hora — la única fuente de verdad es esta nota, no lo que él diga. NO le digas que "no encuentras nada" ni que "el sistema no lo detecta" (suena robótico y desconfiado). Si todavía NO te ha dicho exactamente qué día y hora eligió, pregúntaselo con calidez, como si quisieras anotarlo tú mismo para tenerlo presente. Si YA te dijo el día y la hora (en este mensaje o en uno anterior) y aun así no aparece confirmada, no le vuelvas a preguntar lo mismo — en vez de eso, explícale con calidez y sin sonar acusador que su agenda no quedó completa: es un error común, a veces se entra al calendario y se elige el día, pero falta darle al botón de Agendar y responder las preguntas del formulario, y por eso su espacio no se guardó. Reenvíale el enlace del calendario que ya usas normalmente y anímalo con calidez a completar esos dos pasos para que su llamada quede agendada de verdad.`;
    }
...
<style>
  ...
  .bubble{
    padding:10px; border-radius:13px; font-size:14px; line-height:1.35;
